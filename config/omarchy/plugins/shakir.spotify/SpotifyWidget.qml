...
// Bar-widgets are sandboxed away from omarchy.media (see shell.qml's
// pluginHasBarCapabilities/createScopedPluginShell: firstPartyServiceFor is
// only handed to kind:"bar" or "omarchy.indicators" clones), so this reads
// Quickshell's own Mpris module directly instead of going through that
// service, and reimplements the small slice of its play/pause/skip logic
// needed here against the Spotify player specifically.
BarWidget {
  id: root
  moduleName: "shakir.spotify"

  function isSpotify(player) {
    if (!player) return false
    var dbus = String(player.dbusName || "").toLowerCase()
    var entry = String(player.desktopEntry || "").toLowerCase()
    return dbus.indexOf("org.mpris.mediaplayer2.spotify") !== -1 || entry === "spotify"
  }

  readonly property var activePlayer: {
    var list = Mpris.players ? Mpris.players.values : []
    for (var i = 0; i < list.length; i++) {
      if (isSpotify(list[i])) return list[i]
    }
    return null
  }
  ...
  // Flash the title briefly on every track change, including the very first
  // one Spotify launches into (deliberate: it's a nice "now playing" cue).
  property bool flashActive: false
  onTitleChanged: if (title !== "") { flashActive = true; flashTimer.restart() }
  Timer {
    id: flashTimer
    interval: 900
  ...
  Row {
    ...
    Item {
      ...
      Text {
        ...
        color: root.flashActive ? Color.accent : root.bar.barForeground
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.body
        anchors.verticalCenter: parent.verticalCenter
