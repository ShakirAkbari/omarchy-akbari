...
|                                       on tailscale0, port 3389)    |
|  wayvnc + wayvnc-tailscale    ---->  VNC into the live Hyprland    |
...
separate offers, because on Hyprland no single server does both jobs. See
[Remote desktop from Windows](#remote-desktop-from-windows) below.
...
Hyprland is a Wayland compositor with no RDP support: `gnome-remote-desktop`
and `krdp` need GNOME or KDE hooks it lacks, and `xdg-desktop-portal-hyprland`
does not offer input injection. So there are two options, and install.sh
offers each separately:

| | xrdp | wayvnc |
|---|---|---|
| Protocol | RDP | VNC |
| Windows client | built in (`mstsc.exe`) | a VNC viewer, e.g. TigerVNC |
| What you get | a **new** icewm desktop per login | **this** Hyprland session, the largest monitor |
| Sees the physical screen | no | yes, and shares mouse and keyboard with it |
| Package source | AUR (`xrdp`, `xorgxrdp`) + `icewm` | `extra/wayvnc` |
| Port | 3389 | 5900 |

Both sign in with your Linux username and password, and neither is reachable
from the LAN or Wi-Fi: each opens its port through ufw on `tailscale0` only
(Omarchy's firewall denies everything incoming by default), and wayvnc also
listens on the Tailscale address alone (`bin/wayvnc-tailscale`), so it is
gated twice. xrdp binds every interface, because binding it to the Tailscale
address would make it fail at boot whenever Tailscale comes up late; for it
the firewall rule is the gate. Both need Tailscale on the Windows machine
too, signed in to the same tailnet.

**xrdp.** Open Remote Desktop Connection, enter this machine's Tailscale name
or IP (`tailscale ip -4`), and sign in. The desktop is icewm, started by
`~/.xinitrc` (linked from `config/xrdp/xinitrc`; edit that file to use a
different window manager). It is a separate X11 session because Hyprland can't
run inside xrdp's Xorg. Audio is not redirected (that needs
`pulseaudio-module-xrdp`, which is built for PulseAudio rather than Omarchy's
PipeWire).

**wayvnc.** Connect a VNC viewer that supports VeNCrypt (TigerVNC's viewer
does) to `<tailscale ip>:5900`, and accept the self-signed certificate on
first connect (generated by install.sh into `~/.config/wayvnc/`). It runs as
the user service `wayvnc.service`, started with the graphical session and
retried every 10 seconds until Tailscale has an address. It captures one
...
- `sudo` access, for the Limine edit (if you confirm it), the Plymouth
  boot screen step, and the remote desktop step (packages, services, ufw).
- Tailscale, for the remote desktop step (skipped with a message without it).
